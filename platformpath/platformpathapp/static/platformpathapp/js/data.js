...
function findPath(stationName, startNodeName, endNodeName) {
    ...
    if (station instanceof MasterNode) {
        ...
        station.edges.forEach((edge) => {
            ...
            if (adjacentNodes[edge.nodeFrom.name] === undefined)
                adjacentNodes[edge.nodeFrom.name] = {
                                                    self: edge.nodeFrom,
                                                    neighbors: []};
            adjacentNodes[edge.nodeFrom.name].neighbors.push({
                                                            node: edge.nodeTo, 
                                                            instruction: edge.instructions});

            // this is what the structure looks like                                                
            /* adjacentNodes["mezzanine"] = {
                                            self: mezzanineNode
                                            neighbors: [
                                                {
                                                node: neighbor_1,
                                                instructions: "go up the stairs toward mezz"},
                                                {
                                                node: neighbor_2,
                                                instructions: "go down the stairs toward mezz"}
                                            ]};
            */
        });
        ...
        const startNode = adjacentNodes[startNodeName].self;
        ...
        while (queue.length > 0) {
            ...
            // if the path backtracking is detected, move onto the next item in queue! 
