...
function findPath(stationName: string, startNodeName: string, endNodeName: string): PathStep[] | null {
    ...
    if (station instanceof MasterNode) {
        ...
        const adjacentNodes: Record<string, { self: Node; neighbors: { node: Node; instruction: string | null }[] }> = {};
        ...
        const queue: [Node, PathStep[]][] = [[startNode, [initialStep]]];
...
function getStation(stationName: string, stationsArray: MasterNode[]): MasterNode | null {
